...
class _SaleTabState extends State<SaleTab> {
  ...
  Future<void> _onBarcode(String code) async {
    ...
    try {
      ...
      if (!mounted) return;
      if (product != null) {
        final asCarton = code == product.barcode || (product.isCarton && code != product.secondaryBarcode);
        final added = asCarton
            ? p.addToCartWithQuantity(product, 1.0, asCarton: true)
            : p.addToCart(product);
        if (added) {
  ...
  Future<void> _completeSale() async {
    ...
    try {
      ...
      if (!mounted) return;
      if (completed == null) {
        _showSnack(p.lastSaleError ?? 'تعذّر إتمام الفاتورة');
        return;
      }
                                          ...
                                          Column(
                                            ...
                                            children: [
                                              if (item.discountPercent > 0) ...[
                                                Text(
                                                  '${item.price.toStringAsFixed(2)} $kCurrencySymbol',
                                                  style: Theme.of(context)
                                                      .textTheme
                                                      .bodySmall
                                                      ?.copyWith(
                                              ...
                                              Text(
                                                '${item.discountedPrice.toStringAsFixed(2)} $kCurrencySymbol/${item.isCarton ? 'كرتونة' : (item.isKg ? 'كجم' : 'حبة')}',
                                                style: Theme.of(
                                                  context,
                                      ...
                                      onTap: item.isKg || item.isCarton
                                          ? () {
                                              ...
                                              showDialog(
                                                ...
                                                if (v == null || !context.mounted) return;
                                                final newSubtotal = double.tryParse(v as String);
                                                if (newSubtotal != null && newSubtotal > 0) {
                                                  p.updateCartItemSubtotal(i, newSubtotal);
                                                }
                                              });
                                            }
                                          : null,
