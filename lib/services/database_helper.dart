...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> getDailySales() async {
    final r = await (await database).rawQuery(
      'SELECT DATE(created_at) as day, COALESCE(SUM(total), 0) as total, COALESCE(SUM(total_profit), 0) as profit, COUNT(*) as count FROM sales WHERE is_archived = 0 GROUP BY DATE(created_at) ORDER BY day DESC'
    );
    return r;
  }
  ...
  Future<double> getCurrentProfitTotal() async {
    final r = await (await database).rawQuery(
        'SELECT COALESCE(SUM(total_profit), 0) as t FROM sales WHERE is_archived = 0'
    );
    return (r.first['t'] as num).toDouble();
  }
  ...
  Future<int> archiveSales() async {
    ...
    final totals = await db.rawQuery(
        'SELECT COALESCE(SUM(total), 0) as ts, COALESCE(SUM(total_profit), 0) as tp FROM sales WHERE is_archived = 0'
    );
