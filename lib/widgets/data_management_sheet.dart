            ...
            builder: (context, setInnerState) => Column(
              ...
              children: [
                ...
                CheckboxListTile(
                  ...
                  subtitle:
                  const Text('الاسم، الباركود، السعر، التكلفة، المخزون'),
                  value: products,
                ...
                CheckboxListTile(
                  ...
                  subtitle:
                  const Text('الفواتير، التاريخ، المنتجات المباعة'),
                  value: sales,
                ...
                CheckboxListTile(
                  ...
                  subtitle:
                  const Text('إجمالي المبيعات، الأرباح، صافي الفائدة'),
                  value: financial,
                  onChanged: (v) =>
                      setInnerState(() => financial = v ?? true),
                ),
              ],
            ),
          ),
    ...
    try {
      ...
      if (selected['products'] == true) {
        ...
        try {
          await exportProductsToExcel(
              data.map((m) => Product.fromMap(m)).toList(), tempPath);
          final bytes = await File(tempPath).readAsBytes();
          final saved = await _saveFileDialog(bytes, 'Products_Export_$ts.xlsx');
          if (saved != null) { successCount++; } else { failCount++; }
        } finally {
      ...
      if (selected['financial'] == true) {
        ...
        try {
          ...
          final bytes = await File(tempPath).readAsBytes();
          final saved = await _saveFileDialog(bytes, 'Financial_Report_$ts.xlsx');
          if (saved != null) { successCount++; } else { failCount++; }
        } finally {
          final f = File(tempPath);
          if (await f.exists()) await f.delete();
      ...
      if (successCount > 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تم حفظ $successCount ملف بنجاح')),
        );
      } else if (failCount > 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('لم يتم حفظ أي ملف')),
        );
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _archive() async {
    ...
    if (salesData.isEmpty) {
      messenger.showSnackBar(
        const SnackBar(content: Text('لا توجد عمليات للأرشفة')),
      );
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => Directionality(
        ...
        child: AlertDialog(
          ...
          content: Text('سيتم أرشفة ${salesData.length} عملية مع إنشاء نسخة احتياطية. هل تود المتابعة؟'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('إلغاء')),
            FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('تأكيد الأرشفة')),
          ],
        ),
      ),
    );
    ...
    try {
      ...
      try {
        ...
        final bytes = await File(tempPath).readAsBytes();
        final savedPath = await _saveFileDialog(bytes, 'Archive_$timestamp.xlsx');
        if (savedPath == null) {
      ...
      if (!mounted) return;
      messenger.showSnackBar(
        SnackBar(content: Text('فشلت الأرشفة: $e')),
      );
    } finally {
    ...
    try {
      ...
      if (filePath == null) {
        ...
        if (bytes == null) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('فشل الاستيراد: تعذر الوصول إلى الملف')),
            );
          }
          setState(() => _isImporting = false);
          return;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('إدارة البيانات', style: Theme.of(context).textTheme.titleMedium),
          ...
          FilledButton.tonalIcon(
            ...
            label: Text(_isArchiving ? 'جارٍ الأرشفة...' : 'أرشفة وتوفير مساحة'),
          ),
          const SizedBox(height: 12),
          Text(
            'Excel (.xlsx)',
