...
class _SaleDetailSheetState extends State<SaleDetailSheet> {
  ...
  void initState() {
    ...
    _items = (jsonDecode(widget.sale['items']) as List)
        ...
        .toList();
    _discountAmount = (widget.sale['discount'] as num?)?.toDouble() ?? 0;
  ...
  double get _finalTotal => (_currentTotal - _discountAmount).clamp(0, _currentTotal);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  setState(() {
                    ...
                    if (!_isEditing) {
                      _discountCtrl.text = _discountAmount.toStringAsFixed(2);
                    }
                  });
                },
              ),
                        ...
                        return Padding(
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                flex: 3,
                                child: Text(item.productName, style: theme.textTheme.bodyMedium, maxLines: 1, overflow: TextOverflow.ellipsis),
                              ),
                              SizedBox(
                                width: 54,
                                child: _isEditing
                                  ...
                                  onChanged: (v) {
                                    ...
                                    if (q != null && q > 0) {
                                      if (!item.isKg && !item.isCarton && q != q.roundToDouble()) return;
                                      setState(() {
                                        _items[i].quantity = q;
                                      });
                                    }
          ...
          if (_discountAmount > 0 || _isEditing) ...[
            const SizedBox(height: 4),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('الخصم:', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.red.shade700)),
                if (_isEditing)
                  SizedBox(
                    width: 100,
                    child: TextField(
                  ...
                  Text(
                    '- ${_discountAmount.toStringAsFixed(2)} $kCurrencySymbol',
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('الإجمالي بعد الخصم:', style: theme.textTheme.titleSmall),
                Text(
                  '${_finalTotal.toStringAsFixed(2)} $kCurrencySymbol',
                  style: theme.textTheme.titleMedium?.copyWith(
                    fontWeight: FontWeight.bold,
                    color: theme.colorScheme.primary,
                  ),
            ...
            FilledButton.icon(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => InvoicePreviewScreen(sale: widget.sale),
                  ),
                );
              },
