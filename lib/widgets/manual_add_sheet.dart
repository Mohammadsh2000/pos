...
class _ManualAddSheetState extends State<ManualAddSheet> {
  ...
  void _tryAddToCart() {
    ...
    final effectiveQty = _selectedProduct!.isKg ? qty : qty.floorToDouble();
    final messenger = ScaffoldMessenger.of(context);
    final feedback = FeedbackService.instance;
    final added = p.addToCartWithQuantity(_selectedProduct!, effectiveQty);
    if (added) {
  ...
  Future<void> _addByBarcode() async {
    ...
    final qty = double.tryParse(_qtyCtrl.text) ?? 1;
    final effectiveQty = product.isKg ? qty : qty.floorToDouble();
    final added = provider.addToCartWithQuantity(product, effectiveQty);
    if (added) {
      feedback.play(ScanSound.success);
      Navigator.pop(context);
    } else {
      feedback.play(ScanSound.warning);
      messenger.showSnackBar(
        SnackBar(content: Text('المخزون غير كافٍ لـ ${product.name} (${product.stock.toStringAsFixed(2)} متاح)')),
      );
    }
  }
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text('${prod.price.toStringAsFixed(2)} $kCurrencySymbol | المخزون: ${prod.stock.toStringAsFixed(2)} | ${prod.isKg ? 'كجم' : 'عدد'}'),
                        onTap: () {
                          setState(() {
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Text('${_selectedProduct!.price.toStringAsFixed(2)} $kCurrencySymbol/${_selectedProduct!.isKg ? 'كجم' : 'حبة'}'),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            SizedBox(
                              ...
                              child: TextField(
                                controller: _qtyCtrl,
                                keyboardType: _selectedProduct?.isKg == true
